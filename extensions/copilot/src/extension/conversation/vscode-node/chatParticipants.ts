...
Learn more about [GitHub Copilot](https://docs.github.com/copilot/using-github-copilot/getting-started-with-github-copilot?tool=vscode&utm_source=editor&utm_medium=chat-panel&utm_campaign=2024q3-em-MSFT-getstarted) in [Pointer](https://code.visualstudio.com/docs/copilot/overview). Or explore the [Copilot walkthrough](command:github.copilot.open.walkthrough).`,
	...
	private async switchToBaseModel(request: vscode.ChatRequest, stream: vscode.ChatResponseStream): Promise<ChatRequest> {
		// Local and other BYOK models do not participate in Copilot quota switching.
		// Return before resolving Copilot metadata so account-free requests never
		// attempt to acquire a GitHub token.
		if (request.model.vendor !== 'copilot') {
			return request;
		}

		const endpoint = await this.endpointProvider.getChatEndpoint(request);
		// If it has a 0x multiplier, it is free, so do not switch it.
